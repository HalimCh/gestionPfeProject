...
public boolean create(Etudiant obj) {
    ...
    public boolean update(Etudiant obj) {
        if(find(obj.getIdentifier())) {
            ...
            JOptionPane.showMessageDialog(null, "id etudiant not found");
            return false ; 
        }
...
class EnseignantDAO extends DAO<Enseignant> {
    ...
    public boolean update(Enseignant obj) {
        if (find(obj.getId())){
           ...
           JOptionPane.showMessageDialog(null, "id teacher not found");
            return false ;  
        }
...
class JurysDAO extends DAO<Jurys> {
    ...
    public boolean create(Jurys obj) {
        try {
            ...
            String query = "INSERT INTO jurys (id, rapporteur, examinateur, encadreur, president, invite) VALUES ("
                    + obj.getId() + ", " + obj.getRapporteur() + ", "
                    + obj.getExaminateur()+ ", " + obj.getEncadreur()+ ", "
                    + obj.getPresident() + ", " + obj.getInvite() + ")";
            state.executeUpdate(query);
            return true;
        } catch (Exception e) {
    ...
    public boolean update(Jurys obj) {
        if (find(obj.getId())){
            try {
            Statement state = this.connect.createStatement();
            String query = "UPDATE jurys SET rapporteur = " + obj.getRapporteur() + ", examinateur = "
                    + obj.getExaminateur() + ", encadreur = " + obj.getEncadreur() + ", president = "
                    + obj.getPresident() + ", invite = " + obj.getInvite() + " WHERE id = "
                    + obj.getId();
            state.executeUpdate(query);
            return true;
        } catch (Exception e) {
            ...
            JOptionPane.showMessageDialog(null, "id jurys not found");
            return false ; 
        }
